...
const createDetermineTripsRtCoverage = (cfg) => {
	...
	const determineTripsRtCoverage = async (feedMsg) => {
		...
		const remainingSchedTripInstances = [...activeSchedTripInstances]
		...
		for (let iRtTripInstance = 0; iRtTripInstance < remainingRtTripInstances.length; iRtTripInstance++) {
			...
			const iSchedTripInstance = remainingSchedTripInstances.findIndex(([schedTripDesc]) => {
				return tripDescriptorsMatch(schedTripDesc, rtTripDesc)
			})
			if (iSchedTripInstance < 0) { // no match
				logger.debug({
			...
			const schedTripInstance = remainingSchedTripInstances[iSchedTripInstance]
			...
			remainingSchedTripInstances.splice(iSchedTripInstance, 1) // delete entry
			remainingRtTripInstances.splice(iRtTripInstance, 1) // delete entry
			iRtTripInstance--
		}

		const unmatchedSchedTripInstances = new Set(remainingSchedTripInstances)
		...
		return {
			nrOfActiveScheduleTripInstances: activeSchedTripInstances.length,
			nrOfRtTripInstances: rtTripInstances.length,
