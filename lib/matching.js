...
import {
	queryActiveTrips as queryActiveScheduleTrips,
} from './gtfs-db.js'
...
const createDetermineTripsRtCoverage = (cfg) => {
	...
	const determineTripsRtCoverage = async (feedMsg) => {
		...
		const _activeScheduleTrips = await queryActiveScheduleTrips({
			db: gtfsDb,
			t: Number.isInteger(feedTimestamp) ? feedTimestamp * 1000 : Date.now(),
		})
		// [[TripDescriptor], ...]
		let activeSchedTrips = _activeScheduleTrips.map(tripDesc => [tripDesc])
		...
		const remainingSchedTrips = [...activeSchedTrips]
		const remainingRtTrips = [...rtTrips]
		// todo: improve the current runtime complexity of `sched * rt`, e.g. by using sorted lists
		for (let iRtTrip = 0; iRtTrip < remainingRtTrips.length; iRtTrip++) {
			logger.trace({iRtTrip}, '--')
			const rtTrip = remainingRtTrips[iRtTrip]
			const [
				rtTripDesc,
			...
			const iSchedTrip = remainingSchedTrips.findIndex(([schedTripDesc]) => {
				return tripDescriptorsMatch(schedTripDesc, rtTripDesc)
			})
			if (iSchedTrip < 0) { // no match
				logger.debug({
			...
			const schedTrip = remainingSchedTrips[iSchedTrip]
			const [schedTripDesc] = schedTrip
			logger.debug({
				rtTripDescriptor: rtTripDesc,
			...
			remainingSchedTrips.splice(iSchedTrip, 1) // delete entry
			remainingRtTrips.splice(iRtTrip, 1) // delete entry
			iRtTrip--
		}

		const unmatchedSchedTrips = new Set(remainingSchedTrips)
		const unmatchedRtTrips = new Set(remainingRtTrips)
		logger.info({
			feedTimestamp,
			activeSchedTrips: activeSchedTrips.length,
			rtTrips: rtTrips.length,
			unmatchedSchedTrips: unmatchedSchedTrips.size,
			unmatchedRtTrips: unmatchedRtTrips.size,
		}, 'matching done')
		return {
			nrOfActiveScheduleTrips: activeSchedTrips.length,
			nrOfRtTrips: rtTrips.length,
			unmatchedRtTrips,
			unmatchedSchedTrips,
		}
	}
